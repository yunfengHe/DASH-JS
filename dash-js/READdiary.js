...
mediasourcebuffer is the play out buffer.  //$$ this understanding is wrong. mediasourcebuffer is an object inherit from basebuffer. basebuffer is more a base class.
...
modifies Dash.js for different segment length
dash.js //myBuffer = init_timeBuffer(2,10,0,video); second field

timebuffer.js   _timeBuffer.initBufferArray("seconds", 10);

DASHttp.js  buffer.push(data, 10);

mediaSourceBuffer.js    _mediaSourceBuffer.initBufferArray("seconds",10);    this.mediaElementBuffered += 10;        
 if(object.mediaElementBuffered < 10) {
           
            rc = object.drain("seconds",10);
...
overlayBuffer = init_mediaSourceBuffer("0", 20,30,0,dashInstance.videoTag); //##this is of importance!! 20: startlevel, 30 maximum level, in seconds  //$$ the third argument buffersize, is passed as maxseconds defined in basebuffer.
...
acutally, this eventhandler is called when mediasourcebuffer does its refill.
...
overlay buffer is not the real video buffer used by the player in mediasource api's hidden blackbox. there's no interface to get the buffer state in the blackbox, what the javascript can do is to make another layer of buffer, to buffer received segments from http responds. and mimic the process of 
video buffer being drained and refilled. the push and request process is controlled by reading the mimiced state of this intermediate buffer!!



may 29th, 2014

addEventlistener is the interface we can use to co work with mediasource API(javascript interpretor)




june 9th, 2014 ;  in adaptationlogic.js, switchRepresentation() function inside init_rateBasedAdaptation()
...
function init_rateBasedAdaptation(_mpd, video, bandwidth)
{
	...
	rateBasedAdaptation.prototype.switchRepresentation = function (){
			...
			this.mpd.period[0].group[0].representation.forEach(function(_rel){
				...
				i++; //$$ try to get a higher rate at first
