...
//&&when the mpd file is responded, parse and callback(after MPD file saved in pmpd successfully, hand over the job to function DASH_MPD_loaded(), it will proceed based on the MPD file- One MPD, all needed info within!!)
MPDLoader.prototype._loadMPD = function()//$$An event handler. when readystate right, load the received MPD file(XML format) and parse it(parse the response content),have the parsed information saved in pmpd, then call the callback function registered ,which is DASH_MPD_loaded()
{
	if(instance.xmlHttp.readyState !=4) return;//&& state=4 response ready
	bps = endBitrateMeasurement(instance.xmlHttp.responseText.length);//&& take the currenttime and calculate estimated bps.
...
MPDLoader.prototype.loadMPD = function(mpdURL)//$$ load the URL of the mpd and try fetch the mpd through http request
{
	...
	beginBitrateMeasurement();//$$ defined in rate_measurement.js, take a initial time as the startT for rate measurement, right after sending mpd request, record the time and save it in measurement.startTimeMeasure
}		
