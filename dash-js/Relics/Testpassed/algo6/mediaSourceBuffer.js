...
function mediaSourceBuffer(id)
{
  ...
  this.doRefill = false;
	this.id = id;
	this.MSABrefillthreshold = 4 //&& media source api 's internal buffer shall have at least several seconds' data, too low, may pause playing because of key frame missing, this unit is in second
	this.Feedingfrequency = 100 //&& unit in millisecond, keep this too low then cpu will be consumed!!  
	this.timecounter = 0;
	this.counterCycle = 5; //&& everytime of feeding mediasource api, increment the counter. every counterCycle times, record buffer state. 
}
...
function init_mediaSourceBuffer(bufferId, criticalLevellow, criticalLevelhigh, buffersize, mediaAPI, videoElement, playbackTimePlot)
{
	...
	mediaSourceBuffer.prototype.bufferStateListener = function(object){ 	
        ...
        * currenTime is a property of HTML Audio/Video DOM , it's in seconds        
				***************************************************************************/		
        object.mediaElementBuffered -= dashPlayer.videoTag.currentTime - object.lastTime; //&& the amount subtracted from mediaElementBuffered, is the amount consumed by the video player        
        ...
        * it has to correspond with the segment length, check basebuffer.js to see the prototype.   
            ...
            if (rc == -1) //&& drain returns -1, indicating end of stream and no data in overlaybuffer.
            {
                ...
                //--dashPlayer.videoTag.webkitSourceEndOfStream(HTMLMediaElement.EOS_NO_ERROR);//-- && I comment this line out; now in use again on july 31th
                return;
            }
        ...
        /********************************************************************************************
 			  * call the buffer checker every 100 ms seems too much, we do this once every 10 cycles
  			********************************************************************************************/        
        this.timecounter += 1; //&& //++
        if(this.timecounter == this.counterCycle ){ 
        	object.callEventHandlers();//++ //&& every Feedingfrequency long, check overlay buffer level and write into file, the registered handler is defined in dash.js
        	this.timecounter = 0;
        }
  ...
  ************************************************************************/ 
  mediaSourceBuffer.prototype.callback = function(){
	...
	mediaSourceBuffer.prototype.getFillLevel = function() //&& get the buffer fill percentage
	{
		return this.state("seconds");
	}
	
	mediaSourceBuffer.prototype.getAbsoluteFillLevel = function() //&& my added prototype.
	{
		return this.Absolutestate("seconds");
	}
