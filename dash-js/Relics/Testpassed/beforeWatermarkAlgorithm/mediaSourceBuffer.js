...
//$$ This is a factory that returns a mediaSourceBuffer object!!
function init_mediaSourceBuffer(bufferId, criticalLevel,buffersize, mediaAPI, videoElement, playbackTimePlot)
{
	...
	//$$ a newly defined eventhandler register, it is used in refilling function to generate debugging info. the handler function is added in dash.js
	...
	//$$ bufferStateListener checks if there's data in overlay buffer, and try feed mediasource api with data from the overlay buffer, every 100 millisecond. it is called in the eventhandlers.js
	//&& mediaSource.bufferStateListener periodically feed mediasource api data, from the overlay buffer, keep an eye on the 	'if(object.mediaElementBuffered < 5)' condition, make sure the value is bigger than 3 and not too big. 
	mediaSourceBuffer.prototype.bufferStateListener = function(object){ //$$ object.drain here is the basebuffer.js's basebuffer.prototype.drain. since mediaSourceBuffer is the child class, then it shall be passed in(as an object). 
		
        object.mediaElementBuffered -= dashPlayer.videoTag.currentTime - object.lastTime; 
        
        //$$ (dashPlayer.videoTag.currentTime - object.lastTime) in reality must be smaller than the segment length, this sentense calculate the time remained before mediasource_api run out of data(before playback broken)
        ...
        //&& originally: if(object.mediaElementBuffered < 1), or try if(object.mediaElementBuffered < seglength)
        //$$ seglength*2 or seglength making this value larger to ensure better reliability, it should be ok to make this different than the segment length, I will try 2 for 1s segment setting. it is actually a threshold. 0.9 s might be too short for feeding the about-to-starve buffer
//&&!! important bug fix!! it mediasource api has a internal buffer, there must be key frame, and the internal player play the video successcively based on time frame, other than segment length, segment is just a bulk of data!!
//&&!! set this value too low, then media player may run out of key frame to play the video, and dashPlayer.videoTag.currentTime would stop to refresh, then it goes into a dead loop. too high, there's just too much to buffer in the browser, no good 
//&& MSABrefillthreshold  represents media source api internal buffer refilling threshold     
        if(object.mediaElementBuffered < this.MSABrefillthreshold) { 
            rc = object.drain("seconds",seglength); 
            //$$ drain(get) data from overlay buffer, give the data piece to rc(pointing rc to it) , the amount drained is one unit of the buffer array, which is the segment length in byte or in seconds. it has to correspond with the segment length, check basebuffer.js to see the prototype.
            
            if (rc == -1)//$$ rc is the return value of drain, it indicates if there's still data in the overlay buffer(mediasource buffer)
            {
                // signal that we are done!
                //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$              
                myFplot.closefile();
                //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$  
                //dashPlayer.videoTag.webkitSourceEndOfStream(HTMLMediaElement.EOS_NO_ERROR);//$$$$$$$$$$$$$$$$$$$$$$$$$$$ I disabled this!!
                //$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$$
                return;
            }
            ...
            if (rc != 0)
            {
                ...
                //$$ feed one unit of data to media_source_api
                this.mediaElementBuffered += seglength;
                //$$ += unit length(segment length)
        ...
        } //$$ if already less than 2, finish it
        object.lastTime = dashPlayer.videoTag.currentTime;
      
        window.setTimeout(function () {_mediaSourceBuffer.bufferStateListener(_mediaSourceBuffer);},this.Feedingfrequency); //&& the timer is set everytime mediaSource.bufferStateListener is called, so it will repeat itself every 100 millisecond. setTimeout excute the timer only once
        //$$The setTimeout() method calls a function or evaluates an expression after a specified number of milliseconds.
        //$$ call bufferStateListener every 0.1 second. every 0.1 second, try feed media_source_api with new data, the purpose of this, is to make sure that mediasource api do not run out(feed it with data from mediasourcebuffer(our overlay buffer)); 
  ...
  //$$ used in DASHttp.js, when called, do refill.
  mediaSourceBuffer.prototype.callback = function(){
	...
	mediaSourceBuffer.prototype.refill = function(object){
        ...
        if(object.doRefill == true){
            ...
            if(object.fillState.seconds < (object.bufferSize.maxseconds-1)){//&& make sure no overlap in the overlaybuffer , namely mediaSourceBuffer happens. I add -1 here.
                ...
                _dashFetchSegmentAsynchron(object);	//$$ the object passed in is the overlay buffer _mediaSourceBuffer //&& this function will enter the adaptation logic and then send http request, when data received, use the callback to refill the overlay buffer with data.
                ...
                object.doRefill = false;//&& when buffer full, stop filling
            }
		}
	}
