...
var maxBandwidth = 6 * 1024 * 1024;        // 4 Mbps
...
function bandwidth(initial_bps, weight_f, weight_s)//&& first instance is in dash.js, in function DASH_MPD_loaded()
{
	...
	this.weight_f = weight_f; //&& weight of the former calculated bps
	this.weight_s = weight_s; //&& weight of the latest measured bps
...
bandwidth.prototype.calcWeightedBandwidth = function(_bps) { 
	...
	//-- this.bps = parseInt(((this.weight_f * this.bps) + (this.weight_s * _bps)) / 2) * 0.9;  // the weights are used to mimic optmistic or pessimistic behavior	//&& (original and too modest) 
	this.bps = parseInt(((this.weight_f * this.bps) + (this.weight_s * _bps)) / 2) * 1;  // the weights are used to mimic optmistic or pessimistic behavior	//&& (original)
